...
#mnisty <- tf$contrib$learn$datasets$mnist
#data <- mnisty$load_mnist()

#feature column
#mnistm <- dataset_mnist()
#mnist_dataset <- tensor_slices_dataset(sources$train$x)
#spec <- feature_spec(mnist$train$x, target ~ .)
my_feature_columns = tf$feature_column$numeric_column("x", shape = shape(784L))
#spec <- spec %>% 
#  step_numeric_column(x, shape = 784L) 
...
classifier <- dnn_classifier(
  ...
  hidden_units =  c(64, 32),
    #feature_columns(
    #tf$feature_column$numeric_column("x", shape = shape(784L))
    #column_numeric("x", shape = shape(784L))
    
  #),
  input_layer_partitioner = min_max_variable_partitioner,
